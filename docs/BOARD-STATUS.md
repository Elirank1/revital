...
- FIX(lead → kanban-ui or V2 owner, NON-BLOCKING): the legacy V2 header (`src/components/Layout/Header.tsx`) is not mobile-responsive — at 390px the icon nav (7 buttons) + dark toggle + status badges lay out ~547px wide, so EVERY view (legacy included) horizontally scrolls on phones. V3 surfaces themselves fit 390px (e2e-asserted inside `<main>`); the page-level scroll check is a `test.fixme` in `e2e/mobile-today.e2e.ts` waiting on this fix. Pre-existing V2 behavior, slightly worsened by the לוח nav entry when the flag is on.
